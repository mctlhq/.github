...
def check_run_gap_against_schedule(declared: str, workflow: pathlib.Path) -> list[str]:
    """Resolve `longest_run_gap` against the cron it claims to come from.

    Deleting the key is refused and lowering it was not: every window clears a
    smaller floor, no message moves, and the file still reads as armed. This
    is the one declared fact here whose source is checkable ten lines away,
    and `unlocks:` already established resolving a reference rather than
    trusting it.
    """
    try:
        text = workflow.read_text()
    except OSError as exc:
        return [f"defaults: longest_run_gap cannot be checked — {exc}"]
    crons = CRON_LINE.findall(text)
    if not crons:
...
def reconcile(gh: GitHub, state: dict, now: dt.datetime) -> dict:
    ...
    return {
        ...
        # previous snapshot. The page's header claims this date is the last
        # state CHANGE, and until now nothing computed one: the property was
        # held entirely by the workflow's `if: changed == 'true'` commit gate,
        # so a run that changed nothing but the timestamp published a page
        # saying the state had changed. A claim the data does not carry is a
        # claim the next caller can break without noticing.
...
def render_markdown(snapshot: dict) -> str:
    out = [
        ...
        "",
        (f"State last changed {snapshot['state_changed_at']} — "
         f"**{snapshot['overall']}**."
         if snapshot.get("state_changed_at") else
         # No baseline: the honest sentence is about this reading, not about a
         # change nobody observed. The `or` that used to stand here is the one
         # carry_state_changed_at's docstring condemns.
...
def selftest() -> int:
    ...
    check("State last changed" in page or "no earlier snapshot" in page,
    ...
    # The page's date must be a property of the data, not of one caller's
    # commit gate.
    base = {"generated_at": "2026-09-12T10:00:00Z",
            "state_changed_at": "2026-09-10T06:00:00Z",
            "overall": DRIFT, "counts": {OBSERVATION_FAILED: 0, DRIFT: 1,
                                         UNEXPECTED_SILENCE: 0, ALIGNED: 0},
            "items": [{"id": "x", "title": "X", "phase": "now", "state": DRIFT,
                       "issue": None, "epic": None, "note": None,
                       "unlocks": [], "reasons": ["r"], "reason_keys": ["r"],
                       "evidence": {}}]}
    check("2026-09-10" in render_markdown(base),
          "the page renders the run time where it claims the change time")
    ...
    check(carry_state_changed_at(None, snap_now, False) is None,
          "a first run claimed a change time it cannot know")
    check(carry_state_changed_at(None, snap_now, True) is None,
          "no baseline is not 'the state changed'")
    ...
    # The declared gap is resolved against the cron it claims to come from,
    # because deleting the key was refused and lowering it was not.
    check(widest_cron_gap("0 6,11,16,21 * * *") == 9 * 3600,
    ...
    with _tempfile.TemporaryDirectory() as d:
        wf = pathlib.Path(d) / "w.yml"
        wf.write_text('on:\n  schedule:\n    - cron: "0 6,11,16,21 * * *"\n')
        check(check_run_gap_against_schedule("9h", wf) == [],
              "the true gap was refused")
        check(any("widest interval" in x
                  for x in check_run_gap_against_schedule("5h", wf)),
              "a lowered constant passed against the real cron")
        check(any("cannot be checked" in x
                  for x in check_run_gap_against_schedule("9h", pathlib.Path(d) / "no.yml")),
              "an unreadable schedule was treated as agreement")
        nocron = pathlib.Path(d) / "n.yml"
        nocron.write_text("on:\n  workflow_dispatch:\n")
        check(any("declares no cron" in x
...
def carry_state_changed_at(before: dict | None, snapshot: dict,
    ...
    """
    if not before:
        # No baseline is not "the state changed": it is not knowing. `changed`
        # defaults to true without one because that is right for the announce
        # decision, and routing a claim about history through the other
        # consumer's boolean is how this shipped a false date twice.
        return None
    if changed:
...
def main(argv: list[str] | None = None) -> int:
    ...
    declared_gap = (state.get("defaults") or {}).get("longest_run_gap")
    if declared_gap and args.schedule:
        problems += check_run_gap_against_schedule(
            declared_gap, pathlib.Path(args.schedule))
    if problems:
